...
public class QueriesSegnalazionePassaggioLivello {
    ...
    //che altro non sono che le query che posso fare sul mio oggetto levelCrossing Illuminazione
    ...
    static final String QUERIES_CERCA_levelCrossing="SELECT * FROM levelCrossing WHERE (codicePL=? AND localizzazione=?);";
    static final String QUERIES_MOSTRA_SEGNALAZIONI_EFFETTUATE="SELECT DISTINCT codicePL,localizzazione,problematica, stato FROM levelCrossing,account WHERE (levelCrossing.codiceUtente=? AND problemaRisolto=false);";
    static final String QUERIES_MOSTRA_TUTTE_SEGNALAZIONI_semafori = "SELECT * FROM levelCrossing;";
    //provala questa delle completate prima
    static final String QUERIES_MOSTRA_SEGNALAZIONI_COMPLETATE="SELECT DISTINCT codicePL,localizzazione,problematica,stato FROM levelCrossing,account WHERE (levelCrossing.codiceUtente=? AND problemaRisolto!=false);";
